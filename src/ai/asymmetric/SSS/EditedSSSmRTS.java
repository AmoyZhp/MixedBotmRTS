 ...
 * @author rubens
 */
public class EditedSSSmRTS extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public EditedSSSmRTS(UnitTypeTable utt) {
        this(100, -1, 200, 4, 4,
//                new SimpleSqrtEvaluationFunction3(),
    ...
    public EditedSSSmRTS(UnitTypeTable utt, List<AI> scripts) {
        this(100, -1, 200, 4, 4,
    ...
    public EditedSSSmRTS(UnitTypeTable utt, List<AI> scripts, String tuplaIndSc) {
        this(100, -1, 200, 4, 4,
                //new CombinedEvaluation(),
                new SimpleSqrtEvaluationFunction3(),
                //new SimpleSqrtEvaluationFunction2(),
    ...
    public EditedSSSmRTS(int time, int max_playouts, int la, int a_I, int a_R, EvaluationFunction e, UnitTypeTable a_utt, PathFinding a_pf, List<AI> scripts) {
    ...
    public EditedSSSmRTS(UnitTypeTable utt, List<AI> scripts, String tuplaIndSc, int qtdPlayout, int lookahead) {
    ...
    public EditedSSSmRTS(int time, int max_playouts, int la, int a_I, int a_R, EvaluationFunction e, UnitTypeTable a_utt, PathFinding a_pf) {
        super(time, max_playouts);
    ...
    public PlayerAction getBestActionSoFar() throws Exception {
        ...
        long time_limit = end_time - this.start_time;
        if(time_limit > TIME_BUDGET){
            //System.out.println("SSSm get Best Action so far time out");
    ...
    protected AI getSeedPlayer(int player) throws Exception {
        ...
        for (AI script : scripts) {
            ...
            if ((System.currentTimeMillis() - start_time) > (TIME_BUDGET - 5)) {
                //System.out.println("seed is time out");
                return seed;
            }
        }
    ...
    public double eval(int player, GameState gs, AI aiPlayer, AI aiEnemy) throws Exception {
        ...
        while (!gameover && gs2.getTime() < timeLimit) {
            end_time = System.currentTimeMillis();
            time_limit = end_time - this.start_time;
            if(time_limit > TIME_BUDGET){
//                System.out.println("eval time out");
                break;
            }
            if (gs2.isComplete()) {
                gameover = gs2.cycle();
